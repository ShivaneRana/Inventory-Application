...
const validationObject = [
    body("manga_name").trim().notEmpty()
    .withMessage("Name cannot be empty"),
    body("manga_rating").trim()
    .isFloat({min:0,max:10})
    .withMessage("Rating can only contain numerical or floating value between 0.1 or 10"),
    body("manga_price").trim()
    .isFloat({min:0.1})
    .withMessage("Price can only contain numerical or floating value."),
    body("manga_chapter_number").trim()
    .isInt({min:1})
    .withMessage("Chapter number can only contain numerical value"),
    body("manga_volume_number").trim()
    .isInt({min:1})
    .withMessage("Volume number can only contain numerical value"),
    body("manga_quantity").trim()
    .isInt({min:1})
...
exports.postAddManga = [
    ...
    if(!errors.isEmpty()){
        const rows = await db.getAllMangas();
        return res.status(400).render("mangas",{rows: rows,flag:true,errors:errors.array()});
    }
